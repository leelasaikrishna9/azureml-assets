...
parser.add_argument('--tb-logs', type=str, default="./logs",
                    help='location to store tensorboard logs')
...
def train(epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if batch_idx % args.log_interval == 0:
            ...
            if hvd.rank() == 0:
                writer.add_scalar('Train/Loss', loss.item(), 100. * batch_idx / len(train_loader))
...
def test():
    ...
    if hvd.rank() == 0:
        ...
        writer.add_scalar('Test/Accuracy', 100. * test_accuracy)
